...
function leaveResourceEditor() {
  ...
  resourceEditor.code?.destroy();resourceEditRequest++;resourceEditor=null;$('#resource-detail').dataset.key='';return true;
}
function editorBody() {
  const ed=resourceEditor;
  if(ed.kind==='skill'&&ed.current)ed.files[ed.current]=ed.code.value;
  ...
  else{body.config=JSON.parse(ed.code.value);body.authentication=$('#editor-auth').value;}
  return body;
}
async function editResource(id=null,kind='skill',source=null) {
  ...
  if(ed.kind==='skill'){
    $('#editor-file').onchange=()=>{ed.files[ed.current]=ed.code.value;ed.current=$('#editor-file').value;ed.code.open(ed.current,ed.files[ed.current]);};
    $('#editor-add-file').onclick=()=>{const name=prompt('File name, relative to this skill:');if(!name)return;if(ed.files[name]!=null){toast('That file already exists',true);return;}ed.files[ed.current]=ed.code.value;ed.files[name]='';ed.current=name;$('#editor-file').insertAdjacentHTML('beforeend',`<option>${esc(name)}</option>`);$('#editor-file').value=name;ed.code.open(name,'');ed.dirty=true;};
    $('#editor-remove-file').onclick=()=>{if(ed.current.toLowerCase()==='skill.md'){toast('SKILL.md is required',true);return;}const removed=ed.current;ed.files[removed]=null;$('#editor-file').selectedOptions[0].remove();ed.current=$('#editor-file').value;ed.code.open(ed.current,ed.files[ed.current]||'');ed.code.forget(removed);ed.dirty=true;};
  }
  $('#save-resource').onclick=async()=>{
    try{
      const body=editorBody();ed.busy=true;ed.code.setReadOnly(true);$('#resource-detail').querySelectorAll('input,select,textarea,button').forEach(el=>el.disabled=true);$('#editor-status').textContent='Saving…';
      ...
      ed.code.destroy();resourceEditor=null;state.resourceId=result.id;state.toolTab=ed.kind;state.resourceSearch='';$('#resource-search').value='';$('#resource-detail').dataset.key='';await loadResources();selectResource(result.id);toast('Saved. Reconnect existing harness connections to load the update.');
    }catch(error){if(resourceEditor===ed){ed.busy=false;ed.code.setReadOnly(false);$('#resource-detail').querySelectorAll('input,select,textarea,button').forEach(el=>el.disabled=false);$('#editor-status').textContent=error.message;}}
  };
  if(id)ed.code.focus();else $('#editor-name').focus();
}
...
function renderResourceDetail(animate=false) {
  ...
  const components=state.tools.resources.filter(x=>(r.components||[]).includes(x.id)||x.plugin===r.id);
  detail.innerHTML=`<button class="inline-action resource-back" id="resource-back">‹ Resources</button><div class="resource-detail-content"><div class="resource-identity"><span class="resource-emblem">${resourceIcon(r.kind)}</span><span>${esc(resourceKinds[r.kind]||r.kind)}${r.discovery==='cached'?' · Cached':''}</span></div><h2 id="resource-detail-title" tabindex="-1">${esc(r.name)}</h2>${['skill','mcp'].includes(r.kind)?`<div class="resource-edit-actions"><button class="inline-action" data-edit-resource="${esc(r.id)}">${r.managed?'Edit':'Edit in Perch'}</button>${r.managed?`<button class="inline-action" data-remove-resource="${esc(r.id)}">Remove…</button>`:''}</div>`:''}${r.managed&&r.kind==='mcp'&&r.authentication==='oauth'?'<div class="resource-auth"><span id="resource-auth-status" role="status">Checking sign-in…</span><button class="inline-action" id="resource-sign-in">Sign in</button><button class="inline-action" id="resource-sign-out" hidden>Sign out</button><button class="inline-action" id="resource-cancel-auth" hidden>Cancel</button></div>':''}${r.plugin?`<button class="inline-action resource-parent" data-open-resource="${esc(r.plugin)}">${esc(state.tools.resources.find(x=>x.id===r.plugin)?.name||'Plugin')}</button>`:''}<div class="detail-section"><h3>${r.kind==='plugin'?'Components':'Harnesses'}</h3>${r.kind==='plugin'?(components.length?components.map(c=>`<button class="navigation-row" data-open-resource="${esc(c.id)}"><span>${esc(c.name)}<small>${esc(resourceKinds[c.kind]||c.kind)}</small></span><span class="row-chevron" aria-hidden="true">›</span></button>`).join(''):'<p class="detail-muted">No portable components detected.</p>'):state.tools.targets.map(t=>{
    ...
    const content=`<span class="harness-monogram" aria-hidden="true">${esc(t.name.slice(0,1))}</span><span class="harness-connection-label"><strong>${esc(t.name)}</strong>${['blocked','disabled','unsupported'].includes(c.status)&&c.reason?`<small>${esc(c.reason)}</small>`:''}</span><span title="${esc(c.reason||'')}" class="connection-state ${c.status==='present'?'is-present':''}">${c.status==='present'?'<i aria-hidden="true"></i>':''}${esc(c.status==='present'&&r.kind==='mcp'?'Configured':labels[c.status]||c.status)}${actionable?'<span aria-hidden="true"> ›</span>':''}</span>`;
...
function showPlan(r) {
  ...
  const issues=[...(r.skills.conflicts||[]),...(r.skills.errors||[]),...(r.skills.skipped||[]),...(r.skipped||[]),...(r.discoveryErrors||[]),...(r.mcp.conflicts||[]),...(r.mcp.blocked||[]),...(r.mcp.errors||[])].filter((item,index,all)=>all.findIndex(other=>(other.skill||other.server||other.source)===(item.skill||item.server||item.source)&&other.reason===item.reason)===index);
